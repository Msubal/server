...
//        String[] str = getResources().getStringArray(R.array.DayOfWeek);
//        ArrayAdapter<String> adapter1 = new ArrayAdapter<String>(getBaseContext(),R.layout.spinner_item,str);
...
//        spinner1.setAdapter(adapter1);
        ...
        spinner1 = (Spinner) findViewById(R.id.week);
        adapter1 = ArrayAdapter.createFromResource(this,R.array.DayOfWeek,android.R.layout.simple_spinner_dropdown_item);
        spinner1.setAdapter(adapter1);
        ...
        place.setText(schedule.get(0).getName() + " " + schedule.get(0).getNumber() + " 시간표");
//        place.setBackgroundColor(Color.parseColor("#edacb1"));
...
//            Log.e("time",time+"");
            ...
            data[time][day-1].setText(" "); //수정 x --> 스페이스하나
        }
...
//        }
        ...
        for(int i=0;i<tr;i++){
            ...
            for(int j=0;j<td;j++){
                ...
                textView[i][j].setTextSize(20);
                textView[i][j].setGravity(Gravity.CENTER);

//                if (j % 2 == 0) {
//                    if(i==0){
//                        textView[i][j].setBackgroundColor(Color.parseColor("#E2EFD9"));
//                    }
//                    else if (i%2==0) {
...
//                }
                ...
                if(j==0){
                    if(i==0){
                        ...
                        textView[i][j].setBackgroundColor(Color.WHITE);
                    ...
                    textView[i][j].setTextColor(Color.BLACK);
                    if(massage.equals(" ")){
                        textView[i][j].setBackgroundColor(Color.parseColor("#FFF2CC"));
                    }else
                        textView[i][j].setBackgroundColor(Color.parseColor("#FFFFFF"));
...
//        Log.d("end_time",end_time+"");
        ...
        for(int i=start_time ; i <= start_time+gap ; i++){
               // Log.d("data","i:"+i+"  week:" + week + "  data:  " + data[i][week-1].getText());
                if(data[i][week-1].getText().toString().equals(" "))
                    isfull = 1;
        }
        ...
        if(gap < 0 ){
            ...
            retrofitService.sendOtpSchedule(otpRequest).enqueue(new Callback<OtpResponse>() {
                ...
                public void onResponse(Call<OtpResponse> call, Response<OtpResponse> response) {
                    ...
                    fixthActivity.putExtra("day",week);
                ...
                public void onFailure(Call<OtpResponse> call, Throwable t) {
                    ...
                    Toast.makeText(getApplicationContext(), strweek + " 이전꺼는 선택이 불가능합니다.", Toast.LENGTH_SHORT).show();
                }
            });
        }
    }
